...
afont *afont_load_fp( FILE *fp )
{
  ...
  if(strncmp(tmps, AFONT_HEADER, AFONT_HEADER_SIZE))
    return NULL;

  a = malloc(sizeof(afont));
  ...
  a->glyph_info = malloc(sizeof(struct _afont_glyph) * a->nchars);
...
void afont_dump_char( afont *a, char c )
{
  ...
  buf = malloc(a->glyph_info[ch].bmpx + 1);
  ...
  switch(a->type) {
    ...
    case AFONT_GL_1BIT:
      for(y = 0; y < a->glyph_info[ch].bmpy; y++) {
        unsigned char *bp;
        unsigned char mask;
        for(i = 0, bp = a->glyph_info[ch].bmp + 
