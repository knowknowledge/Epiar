...
int main( int argc, char **argv )
{
  ...
  if(error) {
    fprintf(stderr, "Couldn't initialize freetype, error %d\n", error);
    assert(!error);
  }

  error = FT_New_Face(ftlib, fontpath, 0, &ftface);
  if(error) {
    fprintf(stderr, "Couldn't load font [%s], error %d\n", fontpath, error);
    assert(!error);
  }
  ...
  if(error) {
    fprintf(stderr, 
            "Couldn't set font size to %d, error %d\n", fontsize, error);
    /* assert(!error); */
  }
  ...
  for(i = 0, fi.imgbuf_size = 0; i < fi.nchars; i++) {
    ...
    if(error) {
      fprintf(stderr, "Error loading glyph %d, error %d", i + base, glyphidx);
      assert(!error);
    }
